...
| `@topox/embed` | Framework-agnostic embed: `mountTopoView(el, opts)` mounts a canvas into any DOM element (React bundled inside — hosts need no framework). Handle offers `applyDiff`/`undo`/`autoLayout`/`connectSSE`/…. See [`docs/embedding.md`](docs/embedding.md). |
